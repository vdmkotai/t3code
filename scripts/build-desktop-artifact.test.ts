...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("uses the fixed fork product name regardless of version (fork build)", () => {
    // FORK BUILD: the product name is a fixed, distinct bundle name ("T3 Fork") so the
    // app installs alongside the primary T3 Code install instead of by stage label.
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Fork");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Fork");
  });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.t3tools.t3code.fork3604",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.t3tools.t3code.fork3604</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.t3tools.t3code.fork3604");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/t3code.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        // FORK BUILD: distinct display name + scheme so deep links resolve to this
        // install, not the primary one.
        { name: "T3 Fork", schemes: ["t3code-fork3604"] },
      ]);
